...
def rod_cutting_bottom_up(n, prices):
    ...
    for length in range(1, n + 1):
        ...
        for possible_cut in range(length):
            # If we find a better price replace max value and solution array sol[i] to point to the length that gave
            # us max value since the index of possible cut starts with 0, to get its value we need to do +1 when
            # accessing dp table
